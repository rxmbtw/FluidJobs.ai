...
const { setUserPermission } = require('../middleware/permissions');
...
router.put('/profile', async (req, res) => {
  try {
    ...
    const adminResult = await pool.query('SELECT * FROM users WHERE id = $1 AND role = $2', [id, 'SuperAdmin']);
    ...
    if (email && email !== admin.email) {
      const emailCheck = await pool.query('SELECT id FROM users WHERE LOWER(email) = LOWER($1) AND id != $2', [email, id]);
    ...
    const updateQuery = `UPDATE users SET ${updateFields.join(', ')} WHERE id = $${paramCount} RETURNING id, name, email, profile_picture`;

    const result = await pool.query(
      'UPDATE users SET ${updateFields.join(', ')} WHERE id = $${paramCount} RETURNING id, name, email, profile_picture',
      updateValues
    );
...
router.post('/login', async (req, res) => {
  try {
    ...
    const result = await pool.query('SELECT * FROM users WHERE LOWER(email) = LOWER($1) AND role = $2', [email, 'SuperAdmin']);
  ...
  try {
    ...
    const candidateResult = await pool.query(
      `SELECT cra.*, 
       ...
       LEFT JOIN users sa_req ON cra.requested_by_user_id = sa_req.id
       LEFT JOIN users au ON cra.approved_by_user_id = au.id
       LEFT JOIN users sa_app ON cra.approved_by_user_id = sa_app.id
...
router.post('/approve-candidate-restriction/:id', async (req, res) => {
  try {
    ...
    const superAdminResult = await pool.query('SELECT id, name FROM users WHERE role = $1 LIMIT 1', ['SuperAdmin']);
...
router.post('/unrestrict-candidate/:id', async (req, res) => {
  try {
    ...
    const userResult = await pool.query('SELECT id, name FROM users WHERE id = $1', [unrestricted_by_user_id]);
...
router.get('/policies', async (req, res) => {
  try {
    // Check if ai_policies table exists
...
router.post('/accounts', async (req, res) => {
  ...
  try {
    ...
    console.log('Create Account Request:', { name, status, locations, assignedUsers });
    ...
    if (assignedUsers && assignedUsers.length > 0) {
      console.log('Assigning users:', assignedUsers);
      for (const userId of assignedUsers) {
        console.log('Assigning user ID:', userId, 'Type:', typeof userId);
        
        // Verify user exists before assigning
        const userExists = await client.query('SELECT id FROM users WHERE id = $1', [userId]);
        if (userExists.rows.length === 0) {
          console.warn(`User ID ${userId} does not exist in users table, skipping assignment`);
          continue;
        }
...
// Get role permissions for SuperAdmin - ALL PERMISSIONS VISIBLE
  ...
  try {
    ...
    const allPermissions = [
      ...
      },
      {
            ...
            "description": "Assign Accounts to Users",
            "category": "user_management"
      },
      {
            "name": "view_jobs",
      ...
      {
            "name": "send_invites",
            "description": "Send Candidate Invitations",
            "category": "candidate_management"
      },
      {
            "name": "bulk_import",
      ...
      {
            "name": "view_pipeline",
            "description": "View Hiring Pipeline",
            "category": "pipeline_management"
      },
      {
            "name": "edit_pipeline",
            "description": "Move Candidates Through Pipeline",
            "category": "pipeline_management"
      },
      {
            "name": "edit_job_settings",
            "description": "Edit Job Settings",
            "category": "pipeline_management"
      },
      {
      ...
      {
            "name": "manage_account_users",
            "description": "Assign Users to Accounts",
            "category": "account_management"
      },
      {
            "name": "view_approvals",
      ...
      {
            "name": "track_own_approvals",
            "description": "Track Own Approval Status",
            "category": "approvals"
      },
      {
            "name": "view_reports",
      ...
      {
            "name": "view_recruiter_analytics",
            "description": "View Recruiter Analytics",
            "category": "reporting"
      },
      {
            "name": "export_reports",
      ...
      },
      {
            ...
            "description": "Manage AI Policies",
            "category": "administration"
      },
      {
...
router.post('/create-user', async (req, res) => {
  try {
    ...
    const superAdminId = req.user?.id || 7; // Get from authenticated user, fallback to first SuperAdmin
    ...
    // Create user with secure temporary password (will be replaced when they accept invite)
    const tempPassword = crypto.randomBytes(16).toString('hex');
    const hashedPassword = await bcrypt.hash(tempPassword, 10);

    const result = await pool.query(
      'INSERT INTO users (name, email, role, password_hash, phone, created_at) VALUES ($1, $2, $3, $4, $5, NOW()) RETURNING id',
    ...
    if (customPermissions && Array.isArray(customPermissions)) {
      for (const perm of customPermissions) {
        if (perm.granted) {
          await setUserPermission(userId, perm.name, true, superAdminId);
        }
      }
    }
    ...
    try {
      await pool.query(
        'INSERT INTO user_invites (email, token, role, invited_by, expires_at) VALUES ($1, $2, $3, $4, $5)',
        [email, inviteToken, role, superAdminId, expiresAt]
      );
      ...
      await sendEmail('noreply', {
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <p>Hello ${name},</p>
            <p>A <strong>SuperAdmin</strong> has invited you to join FluidJobs.ai as a <strong>${role}</strong>.</p>
            <p>Please click the button below to accept the invitation and set up your account password:</p>
            <div style="text-align: center; margin: 30px 0;">
              <a href="${inviteLink}" style="background-color: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold; display: inline-block;">Accept Invitation</a>
            </div>
    ...
    await logAudit(superAdminId, 'SuperAdmin', 'USER_CREATED', `Created user: ${name} (${email}) with role ${role}`, 'user', userId, req);
    
    res.json({
      message: 'User created successfully',
      userId: userId,
      tempPassword: tempPassword // Return for backup, but user should use invite link
    });
  } catch (error) {
