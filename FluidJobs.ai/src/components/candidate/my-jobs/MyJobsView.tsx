...
const MyJobsView: React.FC<MyJobsViewProps> = ({ themeState = 'light', initialFilter = 'all', onFilterChange }) => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      setLoading(true);
      try {
        const [published, saved, applied] = await Promise.all([
        ...
        setSavedJobIds(saved.map((j: any) => j.id.toString()));
        setAppliedJobsList(applied);
  ...
  useEffect(() => {
    if (activeFilter === 'all') {
      ...
      setFilteredJobs(savedJobsList);
    } else if (activeFilter === 'applied') {
      const normalizedApplied = appliedJobsList.map((app: any) => app.job || app);
      setFilteredJobs(normalizedApplied);
    } else {
      setFilteredJobs(jobs);
    }
  }, [activeFilter, jobs, savedJobsList, appliedJobsList]);
  ...
  const handleToggleSave = async (jobId: string) => {
    try {
      const isCurrentlySaved = savedJobIds.includes(jobId.toString());

      if (isCurrentlySaved) {
        await jobService.unsaveJob(jobId);
        setSavedJobIds(prev => prev.filter(id => id !== jobId.toString()));
        setSavedJobsList(prev => prev.filter(job => job.id.toString() !== jobId.toString()));
      } else {
        await jobService.saveJob(jobId);
        setSavedJobIds(prev => [...prev, jobId.toString()]);

        // Optimistically add to saved list if we can find it in 'jobs'
        const jobToAdd = jobs.find(j => j.id.toString() === jobId.toString());
        if (jobToAdd) {
          setSavedJobsList(prev => [...prev, jobToAdd]);
        }
  ...
  if (selectedJob) {
    return (
      <div style={{ backgroundColor: bgColor, minHeight: 'calc(100vh - 80px)', padding: '20px 40px', position: 'relative' }}>
        ...
        <JobDetailView
          job={selectedJob}
          isSaved={savedJobIds.includes(selectedJob.id.toString())}
    ...
    <div style={{ backgroundColor: bgColor, minHeight: 'calc(100vh - 80px)', padding: '20px 40px', position: 'relative', overflowY: 'auto' }}>
      ...
      {activeFilter === 'applied' ? (
        <AppliedJobsView
          ...
          onToggleSave={handleToggleSave}
          jobs={appliedJobsList.map((app: any) => app.job || app)}
        />
      ) : activeFilter === 'saved' ? (
        <SavedJobsView
          themeState={themeState}
        ...
        <div style={{ display: 'flex', position: 'relative' }}>
          ...
          <div
            ...
            onScroll={(e) => {
              ...
              setCurrentJobIndex(Math.min(newIndex, filteredJobs.length - 1));
            }}
          >
            <style>
              {`
            ...
            {filteredJobs.map((job) => (
              <div
                id={`job-card-${filteredJobs.indexOf(job)}`}
                key={job.id}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); handleToggleSave(job.id); }} className="w-[29px] h-[29px] rounded-[5px] flex items-center justify-center" style={{ backgroundColor: '#D9D9D9' }}>
                        <Bookmark set="bulk" primaryColor={savedJobIds.includes(job.id.toString()) ? '#4285F4' : 'rgba(19, 15, 38, 1)'} size={20} style={{ width: '16px', height: '20px' }} />
                      </button>
                    </div>
                  </div>
