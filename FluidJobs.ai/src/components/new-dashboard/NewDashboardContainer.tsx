...
const NewDashboardContainer: React.FC<NewDashboardContainerProps> = ({ onBack, isSidebarExpanded = false, jobId: propJobId, jobTitle }) => {
  ...
  const isLimitedView = false; // Removed interviewer restriction - they can see all tabs
  const isJobSettingsReadOnly = viewRole === 'interviewer' || viewRole === 'sales'; // Only Job Settings is read-only for Interviewer and Sales
  const isPipelineReadOnly = viewRole === 'sales'; // Only Sales has read-only pipeline
  ...
  useEffect(() => {
    const fetchJobDetails = async () => {
      ...
      try {
        const token = sessionStorage.getItem('fluidjobs_token') || localStorage.getItem('superadmin_token');
        const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8000'}/api/jobs-enhanced/${effectiveJobId}`, {
        ...
        if (response.ok) {
          ...
          setJobInfo(prev => ({
            ...
            domain: data.job_domain || data.industry || prev.domain,
            workType: data.mode_of_job || data.location || prev.workType,
            schedule: data.job_type || data.employmentType || prev.schedule,
            icon: (data.title || 'JO').substring(0, 2).toUpperCase()
          }));
  ...
  const menuItems = useMemo(() => {
    // All roles can see all tabs now (Interviewer and Sales in read-only mode)
    return [
  ...
  const renderContent = useMemo(() => {
    ...
    switch (effectiveView) {
      ...
      case 'hiring-pipeline':
        return (
          <ErrorBoundary>
            <PipelineBoard onViewProfile={handleViewProfile} candidates={pipelineCandidates} users={users} stages={jobStages} isReadOnly={isPipelineReadOnly} />
          </ErrorBoundary>
        );
      case 'candidate-tracker':
        return (
          <ErrorBoundary>
            <CandidateTracker onAddCandidate={handleAddCandidate} onViewProfile={handleViewProfile} isReadOnly={isPipelineReadOnly} />
          </ErrorBoundary>
        );
      case 'profile':
      ...
      case 'job-settings':
        return (
          <ErrorBoundary>
            <JobSettings onDirtyChange={handleDirtyChange} isReadOnly={isJobSettingsReadOnly} />
          </ErrorBoundary>
        );
      default:
        return (
