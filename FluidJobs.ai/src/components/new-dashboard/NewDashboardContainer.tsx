...
const NewDashboardContainer: React.FC<NewDashboardContainerProps> = ({ onBack, isSidebarExpanded = false, jobId: propJobId, jobTitle }) => {
  ...
  const isLimitedView = originalRole === 'interviewer';
  ...
  useEffect(() => {
    const fetchJobDetails = async () => {
      ...
      try {
        // Use jobs-enhanced directly — no auth required, returns hiring_process + interview_stages
        const token = sessionStorage.getItem('fluidjobs_token') || localStorage.getItem('superadmin_token') || localStorage.getItem('token');
        let data: any = null;

        const jobsRes = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8000'}/api/jobs-enhanced/${effectiveJobId}`, {
        ...
        if (!data) {
          // Fallback to superadmin endpoint (authenticated)
          const superAdminRes = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8000'}/api/superadmin/jobs/${effectiveJobId}`, {
            headers: token ? { 'Authorization': `Bearer ${token}` } : {}
          });
          if (superAdminRes.ok) {
            data = await superAdminRes.json();
          }
        }
        ...
        if (data) {
          setJobInfo(prev => ({
            ...
            domain: data.department || data.job_domain || prev.domain,
            workType: data.location_type || data.mode_of_job || prev.workType,
            schedule: data.employment_type || data.job_type || prev.schedule,
            icon: (data.job_title || data.title || 'JO').substring(0, 2).toUpperCase()
          }));

          // Read job stages — field may be hiring_process, stages, or interview_stages
          const rawStages = data.hiring_process || data.stages || data.interview_stages || [];
          if (Array.isArray(rawStages) && rawStages.length > 0) {
            // Filter out ASSIGNMENT_RESULT if present, normalize format
            const cleanedStages = rawStages
              .filter((s: any) => {
                const name = s.name || s;
                return name !== 'Assignment Result' && name !== 'ASSIGNMENT_RESULT';
              })
  ...
  const fetchCandidates = useCallback(async () => {
    try {
      ...
      const result = await CandidateService.getJobCandidates(effectiveJobId);
  ...
  }, [addError, clearErrors, effectiveJobId]);
  ...
  // Called when JobSettings saves — re-fetches job stages and candidates for real-time pipeline update
  const handleSettingsSaved = useCallback(async () => {
    if (!effectiveJobId) return;
    try {
      const jobsRes = await fetch(
        `${process.env.REACT_APP_BACKEND_URL || 'http://localhost:8000'}/api/jobs-enhanced/${effectiveJobId}`
      );
      if (jobsRes.ok) {
      ...
      console.error('Failed to refresh after settings save:', e);
    }
  }, [effectiveJobId, fetchCandidates]);
  ...
  const renderContent = useMemo(() => {
    ...
    switch (effectiveView) {
      ...
      case 'candidate-tracker':
        return (
          <ErrorBoundary>
            <CandidateTracker onAddCandidate={handleAddCandidate} onViewProfile={handleViewProfile} jobId={effectiveJobId} />
          </ErrorBoundary>
        );
      case 'profile':
      ...
      case 'interview-analytics':
        return (
          <ErrorBoundary>
            <StageAnalytics jobId={effectiveJobId} />
          </ErrorBoundary>
        );
      case 'job-settings':
        return (
          <ErrorBoundary>
            <JobSettings onDirtyChange={handleDirtyChange} onSaveSuccess={handleSettingsSaved} />
          </ErrorBoundary>
        );
      default:
        return (
  ...
  }, [loading, activeView, selectedCandidate, handleViewProfile, pipelineCandidates, users, handleAddCandidate, handleBackFromProfile, handleStageUpdate, handleCandidateUpdate, handleDirtyChange, jobStages, effectiveJobId, handleSettingsSaved]);
