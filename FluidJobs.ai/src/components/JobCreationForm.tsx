...
const JobCreationForm: React.FC<JobCreationFormProps> = ({ onBack, isSuperAdmin = false }) => {
  ...
  const [hiringStages, setHiringStages] = useState<string[]>([
    'Applied',
    'Screening',
    'CV Shortlist',
    'HM Review',
                          ...
                          <div key={index} className="group flex items-center gap-3 p-4 bg-gray-50 rounded-lg border border-gray-200 hover:border-gray-300 transition-all">
                            ...
                            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              <button
                                ...
                                disabled={index === hiringStages.length - 1}
                                className="p-1 text-gray-400 hover:text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                                title="Move down"
                              >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          ...
                          <div className="space-y-3">
                              ...
                              <input
                                ...
                                onKeyPress={(e) => {
                                  if (e.key === 'Enter' && newStageInput.trim()) {
                                    e.preventDefault();
                                    setHiringStages([...hiringStages, newStageInput.trim()]);
                                    setNewStageInput('');
                                  }
                                }}
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {predefinedStages
                                  .filter(stage => !hiringStages.includes(stage))
                                  .slice(0, 6)
                                  .map(stage => (
                                    <button
                                      key={stage}
                                      onClick={() => setHiringStages([...hiringStages, stage])}
