...
const JobGridMobile: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="job-grid-mobile" style={{ display: 'flex', justifyContent: 'center', padding: '40px 0' }}>
        <div style={{
  ...
  if (error || jobs.length === 0) {
    return (
      <div className="job-grid-mobile" style={{ textAlign: 'center', padding: '40px 20px', color: '#6b7280' }}>
        <p>{error || 'No job openings available at the moment.'}</p>
      </div>
    )
  }

  return (
    <div className="job-grid-mobile">
      {jobs.map(job => (
